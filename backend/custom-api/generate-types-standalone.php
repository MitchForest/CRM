 ...
 * This version works without SuiteCRM entryPoint
 */

// Simple autoloader for our DTO classes
...
// First, let's manually generate TypeScript from our DTOs
$typeScriptContent = "/**
...
$frontendDir = __DIR__ . '/../../../frontend/src/types';
if (!is_dir($frontendDir)) {
    if (!mkdir($frontendDir, 0777, true)) {
        die("Failed to create directory: $frontendDir\n");
    }
}

// Write TypeScript file
$typeScriptPath = $frontendDir . '/api.generated.ts';
if (file_put_contents($typeScriptPath, $typeScriptContent) === false) {
    die("Failed to write TypeScript file: $typeScriptPath\n");
}
echo "✓ TypeScript types written to: $typeScriptPath\n";

// Write Zod schemas file
$zodPath = $frontendDir . '/api.schemas.ts';
if (file_put_contents($zodPath, $zodContent) === false) {
    die("Failed to write Zod schemas file: $zodPath\n");
}
echo "✓ Zod schemas written to: $zodPath\n";

echo "\nTypeScript generation complete! 🎉\n";
echo "Generated files:\n";
echo "- " . realpath($typeScriptPath) . "\n";
echo "- " . realpath($zodPath) . "\n";