...
class AuthController extends BaseController {
    ...
    public function refresh(Request $request) {
        ...
        if (!$refreshToken) {
            return Response::error('Refresh token required', 400, ['refreshToken' => 'Required field']);
        }
        ...
        try {
            ...
            if (($payload['type'] ?? '') !== 'refresh') {
                return Response::error('Invalid token type', 400, ['refreshToken' => 'Must be a refresh token']);
            }
            ...
            $query = "SELECT user_name, email1 FROM users WHERE id = '$userId' AND deleted = 0";
            $result = $db->query($query);
